    ...
    if (e.parameter && e.parameter.action === 'getFaq') {
      ...
      try {
        ...
        if (!faqSheet) {
          ...
          return ContentService.createTextOutput(JSON.stringify({success: false, error: 'FAQ sheet not found'})).setMimeType(ContentService.MimeType.JSON);
        }
        ...
        const faqItems = data.slice(1).map(row => ({
          ...
          answer: row[1] || '',
          order: row[2] || 0
        })).filter(item => item.question && item.answer); // Фильтруем пустые строки
        
        // Сортируем по порядку
        faqItems.sort((a, b) => (a.order || 0) - (b.order || 0));
        
        Logger.log('Found FAQ items:', faqItems.length);
        Logger.log('First FAQ item:', faqItems[0]);
        
        return ContentService.createTextOutput(JSON.stringify({success: true, faq: faqItems})).setMimeType(ContentService.MimeType.JSON);
        
      } catch (error) {
        Logger.log('Error getting FAQ:', error.toString());
...
// Функция для тестирования получения FAQ
function testGetFaq() {
