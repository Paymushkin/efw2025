...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "container mx-auto px-5 py-10 md:py-16" }, _attrs))} data-v-8c416527><h1 class="text-3xl md:text-4xl xl:text-5xl mb-6" data-v-8c416527>14th Season Spring Summer 26</h1><p class="text-base md:text-lg text-gray-700 mb-6" data-v-8c416527> Statistics: Total 13 Photobanks. 10.000+ photo. 135+ TV interviews. 400+ Videos </p>`);
      if (tocItems.value.length) {
        _push(`<nav class="mb-10 md:mb-16 rounded-2xl border border-gray-200 bg-gray-50/80 p-4 md:p-6" data-v-8c416527><p class="text-xs font-semibold uppercase tracking-widest text-gray-500 mb-4" data-v-8c416527> Contents </p><ul class="flex flex-wrap gap-2" data-v-8c416527><!--[-->`);
        ssrRenderList(tocItems.value, (item) => {
          _push(`<li data-v-8c416527><a class="inline-flex items-center rounded-full border border-gray-300 bg-white px-3 py-1.5 text-sm text-gray-700 transition hover:border-black hover:text-black"${ssrRenderAttr("href", `#${item.id}`)} data-v-8c416527>${ssrInterpolate(item.title)}</a></li>`);
        });
        _push(`<!--]--></ul></nav>`);
      } else {
        _push(`<!---->`);
      ...
      if (unref(isLoading)) {
        _push(`<div class="flex items-center justify-center py-10 text-gray-500" data-v-8c416527> Loading galleries… </div>`);
      } else if (unref(errorMessage)) {
        _push(`<div class="flex flex-col items-center justify-center gap-4 py-10 text-center text-sm text-red-600" data-v-8c416527><p data-v-8c416527>Не удалось загрузить галерею из Google Sheets.</p><p class="max-w-[480px] text-gray-500" data-v-8c416527>${ssrInterpolate(unref(errorMessage))}</p><p class="text-gray-400" data-v-8c416527>Проверьте доступность таблицы и попробуйте обновить страницу.</p></div>`);
      } else {
        _push(`<div class="photographers-list space-y-12 md:space-y-16" data-v-8c416527><!--[-->`);
        ssrRenderList(unref(photographers), (photographer, index) => {
          _push(`<section${ssrRenderAttr("id", getAnchorId(photographer, index))} class="scroll-mt-28" data-v-8c416527>`);
...
const gallerySs26 = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-8c416527"]]);
export {
  gallerySs26 as default
};
//# sourceMappingURL=gallery-ss-26-DNZoJmBB.js.map
